...
public class EventFragment extends Fragment {
    ...
    private void loadEvent() {
        service.MakeGetRequest(Const.GET_EVENTS + UserProfile.SessionData.Id, getContext(), new VolleyJsonObjectCallback() {
            @Override
                    ...
                    try {
                        if (result.getJSONArray("data") != null) {
                            ...
                            for (int i = 0; i < array.length(); i++) {
                                ...
                                if (json.getJSONArray("event_image") != null) {
                                    event.Images = new ArrayList<EventImage>();
                                    JSONArray images = json.getJSONArray("event_image");
                                    for (int j = 0; j < images.length(); j++) {
                                        image = new EventImage();
                                        JSONObject img = images.getJSONObject(j);
                                        image.Id = img.getInt("id");
                                        image.Id = img.getInt("event_id");
